...
import { BootstrapUtils, LoggerFactory, LogType } from 'symbol-bootstrap';
...
`;
    ...
    public async run(): Promise<void> {
        ...
        const nemesisTargetFolder = await service.generateNemesis(keyStore, { regenerate, password: false });
        logger.info('');
        logger.info('');
        ...
        logger.info(` - Web Wallet - ${NetworkUtils.resolveWalletUrl('localhost')}`);
        logger.info(` - Explorer - ${NetworkUtils.resolveExplorerUrl('localhost')}`);
        logger.info(` - Faucet - ${NetworkUtils.resolveFaucetUrl('localhost')}`);
        logger.info('');
