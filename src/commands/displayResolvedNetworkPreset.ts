...
import { ConfigLoader, CustomPreset, LoggerFactory, LogType, YamlUtils } from 'symbol-bootstrap';
...
It's recommended you only add the customizations for your network in the final  ${NetworkUtils.NETWORK_PRESET_FILE}.
    ...
    public async run(): Promise<void> {
        ...
        const network = (await YamlUtils.loadYaml(NetworkUtils.NETWORK_PRESET_FILE, undefined)) as CustomPreset;
        ...
        logger.info('------');
        logger.info('');
        logger.info(YamlUtils.toYaml(merged));
        logger.info('------');
        logger.info('');
        logger.info(
