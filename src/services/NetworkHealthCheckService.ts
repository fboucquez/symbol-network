...
import { NetworkUtils, NodeInformation, nodesMetadata } from '../';
...
export class NetworkHealthCheckService {
    ...
    private async testRestGateway(
        ...
        try {
            ...
            report.error(`Rest ${testUrl} is NOT up and running: ${NetworkUtils.getMessage(e)}`);
        }
        ...
        try {
            ...
            report.error(`Rest ${url}/chain/info error: ${NetworkUtils.getMessage(e)}`);
        }
        ...
        try {
            ...
            report.error(`Rest ${url}/node/info error: ${NetworkUtils.getMessage(e)}`);
        }
        return report;
    }
    ...
    private async testAccounts(node: NodeInformation, maxNodeData: NodeData, timeout: number): Promise<Report> {
        ...
        try {
            ...
            report.error(`Account ${node.addresses?.main.address} could not be loaded. Error: ${NetworkUtils.getMessage(e)}`);
        }
        return report;
    }
    ...
    private async testServer(node: NodeInformation, timeout: number): Promise<Report> {
        ...
        return new Promise<Report>((resolve) => {
            sock.on('connect', () => {
                ...
                .on('error', (e) => {
                    report.error(`${host} port ${port} is down: ${NetworkUtils.getMessage(e)}`);
                    resolve(report);
                })
                .on('timeout', () => {
