...
import { CryptoUtils, YamlUtils } from 'symbol-bootstrap';
...
export class NetworkUtils {
    ...
    public static async loadNetworkInput(workingDir: string): Promise<NetworkInputFile> {
        ...
        const input = (await YamlUtils.loadYaml(inputFile, undefined)) as NetworkInputFile;
        //Add validation;
        return input;
    }
    ...
    public static async loadNetwork(workingDir: string): Promise<NetworkFile> {
        ...
        const input = (await YamlUtils.loadYaml(inputFile, undefined)) as NetworkFile;
        //Add validation;
        return input;
    }
    ...
    public static async saveNetwork(workingDir: string, network: NetworkFile): Promise<void> {
        ...
        await YamlUtils.writeYaml(outputFile, CryptoUtils.removePrivateKeys(network), undefined);
    }
